...
def pack_articles(db_prod, df):
	try:
		...
		df["neighborhoods"] = df["tracts"].progress_apply(locateNeighborhoods)

		article_df = df.set_index('content_id')
...
def pack_neighborhoods(db_prod, df):
	try:
		...
		df["neighborhoods"] = df["tracts"].progress_apply(locateNeighborhoods)
		neighborhood_list = df['neighborhoods'].to_list()
		tagging_list = df['content_id'].to_list()
...
def pack_topics(db_prod, df):
	try:
		...
		topics_list = df['openai_labels'].to_numpy()
...
def pack_tracts(db_prod, df):
	try:
		...
		if tract_collection_name not in collection_list:
			...
			# addExistingTracts(tract_collection) # We add existing tracts from the old census tracts, DEPRECATED
